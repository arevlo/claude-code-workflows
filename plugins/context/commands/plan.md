...
3. [2024-12-15] Browser Extension Setup
...
2. Notion (_context database)         - Persistent, searchable, tagged
...
**If Notion:**
- Use `mcp__notion__notion-create-pages` to create a new page in `_context` database
- Set properties:
  - Title: Plan title
...
Use `mcp__notion__notion-search` with query in `_context` database, filtered to Plan tag.
...
- **`/context:plan`** - Start of session, pick up where you left off
- **`/context:plan save`** - Before running out of context, preserve your plan
- **`/context:plan latest`** - Quick resume of most recent work
- **`/context:plan search auth`** - Find a specific plan by topic
